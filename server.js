Here’s your **full `server.js` code**, keeping the same structure and format you already had, but with the private key handling fixed so it works correctly on Render:
...
app.post("/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    if (!FOLDER_ID) {
      console.error("[/files] Missing configuration: GOOGLE_DRIVE_FOLDER_ID is not set");
...
app.put("/rename", async (req, res) => {
  try {
    ...
    if (!id)Here’s your **full `server.js` code**, keeping the same format you wrote but with the private key handling corrected so it works properly on Render:
...
async function verifyFolderEditorAccess() {
  ...
  try {
    ...
    if (!match) {
      throw new Error(`Service account ${SERVICE_ACCOUNT_EMAIL} has no direct permission on folder ${FOLDER_ID}`);
    }

    const role = match.role;
    if (!EDITOR_ROLES.has(role)) {
      throw new Error(`Service account role on folder is '${role}' but editor access (writer or higher) is required`);
    }
  } catch (err) {
    logGoogleApiError("FolderPermissionsList", err);
    throw err;
...
app.post("/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (!req.file) {
      return res.status(400).json({ success: false, error: "No file uploaded", reason: "Request must include a file" });
    }
...
app.get("/files", async (req, res) => {
  try {
    ...
    const response = await drive.files.list({ q: `'${FOLDER_ID}' in parents`, fields: "files(id, name)" });
...
app.delete("/delete/:id", async (req, res) => {
  try {
    const fileId = req.params.id;
    if (!fileId) {
      return res.status(400).json({ success: false, error: "Missing file ID", reason: "Request parameter ':id' required" });
    }
    console.log(`[Delete] fileId="${fileId}"`);
    await ensureFolderAccessVerified();
    await drive.files.delete({ fileId });
